...
/**
 * Static class: not recommended for web service
...
public class PrecomputeIndexCombinationStatic {
	...
	//The position of the list is the size of the combinations, then the entry/key of the hashmap is the elements to be combined
	private static List<Map<Integer, Set<Set<Integer>>>> precomputedCombinations = new ArrayList<Map<Integer, Set<Set<Integer>>>>();
	
	
	private static Set<Set<Integer>> maxIdentifierCombination = new HashSet<Set<Integer>>();
	...
	public static Set<Set<Integer>> getMaxCombination(int size){
		maxIdentifierCombination.clear();
		combination.clear();
		for (int i=0; i<size; i++)
			combination.add(i);
		
		maxIdentifierCombination.add(new HashSet<Integer>(combination));
		return maxIdentifierCombination;	
	}
	
	
	public static Set<Integer> getMaxCombinationSet(int size){
		combination.clear();
		for (int i=0; i<size; i++)
			combination.add(i);
