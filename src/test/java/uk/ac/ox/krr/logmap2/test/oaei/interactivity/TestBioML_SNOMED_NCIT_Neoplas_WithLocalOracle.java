package uk.ac.ox.krr.logmap2.test.oaei.interactivity;

import uk.ac.ox.krr.logmap2.test.oaei.OAEITask;

public class TestBioML_SNOMED_NCIT_Neoplas_WithLocalOracle extends TestOAEITrackWithOracle{

	@Override
	protected void setUp() {
		
		
		boolean extended_questions = false;		

		setIputOutputFiles("bio-ml/snomed-ncit.neoplas", "bioml-snomed-ncit.neoplas",  extended_questions);
		
				
		tasks.add(
				new OAEITask(
						URI_PATH + "snomed.neoplas.owl",  //source
						URI_PATH + "ncit.neoplas.owl",  //target
						//URI_PATH + "reference.rdf",   //reference mappings if any
						URI_PATH + "refs_equiv/snomed-ncit.neoplas-ref.rdf", //no reference
						"bioml-snomed-ncit-neoplas"
				));
		
	}
	
	
	public static void main(String[] args){
		
		TestBioML_SNOMED_NCIT_Neoplas_WithLocalOracle test = new TestBioML_SNOMED_NCIT_Neoplas_WithLocalOracle();
		
		
		try {
			test.evaluateTasks();
			
			
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		
		
	}


}
